...
def main(config_path):
    ...
    # generator update below). Its weights stay at their pretrained values, so training stability
    # is maintained. We keep requires_grad=True and train() mode because the alignment it produces
    # is part of g_loss's gradient path — cutting grad or switching to eval() both break the
    # backward. Not optimising it is enough: clean weights -> clean alignment targets, while
    # everything downstream still trains normally.
